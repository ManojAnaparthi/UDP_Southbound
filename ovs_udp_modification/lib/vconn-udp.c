               ...
               uint8_t dscp)
{
    ...
    /* Open UDP stream */
    error = stream_open_with_default_port(suffix, OFP_PORT, &stream, dscp);
    if (error) {
        VLOG_ERR("Failed to open UDP stream '%s': %s",
                 suffix, ovs_strerror(error));
        return error;
    }

    /* Create vconn */
    *vconnp = vconn_udp_new(stream, error);
...
vconn_udp_recv(struct vconn *vconn, struct ofpbuf **msgp)
{
    ...
    error = stream_recv(udp->stream, ofpbuf_data(rx), ofpbuf_tailroom(rx));
    ...
    const struct ofp_header *oh = ofpbuf_data(rx);
...
{
    struct vconn_udp *udp = vconn_udp_cast(vconn);
    size_t msg_len = ofpbuf_size(msg);
    int error;
    ...
    error = stream_send(udp->stream, ofpbuf_data(msg), msg_len);
...
vconn_udp_run(struct vconn *vconn)
{
    ...
    if (udp->txbuf) {
        size_t msg_len = ofpbuf_size(udp->txbuf);
        int error = stream_send(udp->stream, ofpbuf_data(udp->txbuf), msg_len);
...
static int
vconn_udp_get_status(const struct vconn *vconn OVS_UNUSED)
{
    /* UDP is always "connected" once opened */
    return 0;
}
...
const struct vconn_class udp_vconn_class = {
    ...
    NULL,                       /* run_wait */
    vconn_udp_wait,             /* wait */
    vconn_udp_get_status,       /* get_status */
};
