...
static int
new_udp_stream(char *name, int fd, int connect_status,
         ...
         uint8_t dscp OVS_UNUSED)
{
    ...
    /* Create a non-blocking connected UDP socket to the target. */
    error = inet_open_active(SOCK_DGRAM, suffix, -1, &ss, &fd, 0);
    if (fd < 0) {
        VLOG_ERR("%s: connect: %s", name, ovs_strerror(error));
        return error;
    }

    socklen_t ss_len = ss_length(&ss);
    ...
    return new_udp_stream(xstrdup(name), fd, error, &ss, ss_len, streamp);
}
